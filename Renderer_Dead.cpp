...
double Renderer_Dead::probContamination;
...
FunctDistrDisc<int, double> Renderer_Dead::renderFactor;

// Table with 2 entries -- 1) Type of renderer, and 2) probability that animal remains
// will be sent to a renderer of type in (1)
FunctTable<Meal::ProducerType, double> Renderer_Dead::probType;
FunctTable<Meal::ProducerType, double> Renderer_Dead::probMisLabel;
double Renderer_Dead::fracContaminate;

Renderer_Dead::Renderer() {}
	
//Receives a vector of organs (i.e., the carcas) and returns a mealGroup
//Infectivity ends up in P MBM, NP MBM, and in BLOOD
MealGroup Renderer_Dead::process(std::vector<Organ>& organs) {
	...
	if(producer == Meal::MIXED_MBM_PRODUCER && generator.nextFloat() <= probContamination ) {
		double deltaContamination = Renderer_Dead::fracContaminate*mealP.id50;
...
void Renderer_Dead::readError() {
	cerr << "Error reading Renderer_Dead data" << endl;
	exit(0);
}

//input function from data stream
istream& operator>> (istream& stream, Renderer_Dead& me) {

	string word;
	char dump[256];
	stream >> word;

	while((word != "</renderer_Dead>") && !stream.eof()) {
		...
		else Renderer_Dead::readError();
		stream >> word;
	} if(word != "</renderer_Dead>") Renderer_Dead::readError();
	return stream;
}
